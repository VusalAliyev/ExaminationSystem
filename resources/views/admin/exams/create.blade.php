...
@section('content')
    <div class="container-fluid py-4">
        <h1 class="mb-4 text-primary">Yeni İmtahan Əlavə Et</h1>
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        <form action="{{ route('exams.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="exam_type_id" id="exam_type_id" class="form-select" required>
                                    ...
                                    @foreach ($types as $type)
                                        <option value="{{ $type->id }}" data-name="{{ $type->type }}">{{ $type->type }}</option>
                                    @endforeach
                                </select>
                                @error('exam_type_id')
                                <div class="text-danger">{{ $message }}</div>
                            ...
                            <button type="submit" class="btn btn-success w-100">İmtahanı Yadda Saxla</button>
                        </form>
                    </div>
                </div>
            </div>
